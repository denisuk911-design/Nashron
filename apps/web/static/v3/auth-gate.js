(function () {
  ...
  function setMode(next) {
    ...
    $("#auth-name-label").hidden = !(bootstrap || register); $("#auth-language-label").hidden = !(bootstrap || register); $("#auth-account-label").hidden = change; $("#auth-account").required = !change;
  ...
  async function open() {
    ...
    if (token) { try { await request("/api/auth/me", { headers: { Authorization: `Bearer ${token}` } }); initialized = true; ready(); return; } catch (_) { localStorage.removeItem("luminifera.authToken"); } }
