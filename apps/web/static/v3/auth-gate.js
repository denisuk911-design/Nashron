(function () {
  ...
  const copy = {
    ru: { eyebrow:"LUMINIFERA · IRIS", loginTitle:"Войдите в своё рабочее пространство", loginSub:"Ваши команды, цели и проверенные результаты сохранены в профиле.", bootstrapTitle:"Откройте своё рабочее пространство", bootstrapSub:"Создайте первый профиль владельца. Iris встретит вас внутри и поможет начать.", registerTitle:"Создайте профиль Luminifera", registerSub:"После регистрации Iris встретит вас и поможет начать работу.", passwordTitle:"Обновите пароль профиля", passwordSub:"Новый пароль защитит доступ к вашему рабочему пространству.", account:"Email", name:"Ваше имя", password:"Пароль", language:"Язык интерфейса", login:"Войти", bootstrap:"Создать аккаунт", register:"Создать аккаунт", change:"Сохранить пароль", create:"Создать аккаунт", back:"Уже есть профиль? Войти", logout:"Выйти", statusCreated:"Профиль создан. Входим в рабочее пространство…", statusChanged:"Пароль изменён. Войдите снова.", invalidEmail:"Введите корректный email.", nameRequired:"Укажите имя.", shortPassword:"Пароль должен содержать не менее 10 символов.", passwordRules:"Пароль должен содержать буквы и цифры.", disabled:"Регистрация сейчас недоступна. Для первого запуска создайте рабочее пространство.", duplicate:"Профиль с таким email уже существует.", invalid:"Профиль не найден или введены неверные данные.", rate:"Слишком много попыток. Повторите позже.", closed:"Первый профиль уже создан. Войдите с его данными.", network:"Не удалось связаться с Luminifera. Проверьте подключение и повторите попытку.", generic:"Не удалось выполнить действие. Проверьте данные и повторите попытку." },
 ...
 let mode = "login", language = localStorage.getItem("luminifera.authLanguage") || document.documentElement.lang || "ru";
  ...
  function setMode(next) {
    mode = next; const bootstrap = mode === "bootstrap", register = mode === "register", change = mode === "password";
    $("#auth-title").textContent = text(bootstrap ? "bootstrapTitle" : register ? "registerTitle" : change ? "passwordTitle" : "loginTitle");
    $("#auth-subtitle").textContent = text(bootstrap ? "bootstrapSub" : register ? "registerSub" : change ? "passwordSub" : "loginSub");
    $("#auth-name-label").hidden = !(bootstrap || register); $("#auth-account-label").hidden = change; $("#auth-account").required = !change; $("#auth-language-label").hidden = !(bootstrap || register); $("#auth-language").value = language;
    $("#auth-submit").textContent = text(bootstrap ? "bootstrap" : register ? "register" : change ? "change" : "login"); $("#auth-alt").hidden = change; $("#auth-alt").textContent = mode === "login" ? text("create") : text("back"); $("#auth-password").autocomplete = change ? "new-password" : "current-password"; $("#auth-error").hidden = true;
  }
