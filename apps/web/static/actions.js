...
(() => {
  const initSettings = () => {
    ...
    const dialog = document.createElement('dialog');
    dialog.innerHTML = '<form method="dialog" style="padding:28px;display:grid;gap:14px"><button class="dialog-close" value="cancel" aria-label="Закрыть">×</button><span class="eyebrow">Профиль и настройки</span><h2>Рабочая среда</h2><label>Язык интерфейса<select name="interface_language"><option value="ru">Русский</option><option value="uk">Українська</option><option value="en">English</option></select></label><label>Тема<select name="theme"><option value="dark">Тёмная</option><option value="light">Светлая</option><option value="night_city">Ночной город</option></select></label><label style="display:flex;gap:8px;align-items:center"><input type="checkbox" name="sound">Звуки сообщений</label><button class="primary" value="default">Сохранить</button><button type="button" class="secondary" id="download-profile-backup">Скачать резервную копию</button><small class="settings-result" style="color:var(--muted)"></small></form>';
    document.body.append(dialog);
    const form = dialog.querySelector('form'); const result = dialog.querySelector('.settings-result');
    form.querySelector('#download-profile-backup').addEventListener('click', () => {
      const link = document.createElement('a');
      link.href = `${window.LUMINIFERA_API_BASE || ''}/api/profile/backup`;
      link.download = 'luminifera-profile.zip';
      link.click();
    });
