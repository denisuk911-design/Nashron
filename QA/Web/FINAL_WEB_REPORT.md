...
LANDING_STATUS: Implemented using supplied Iris poster and animation assets; responsive commercial art direction; value, workflow, capabilities, trust, pricing-direction and CTA sections are present
...
REALTIME: WebSocket `/api/events` publishes organization, Iris and goal lifecycle events; goal execution forwards Runtime V3 checkpoint traces off the event loop as work/artifact/review events; connections are organization-scoped
ORG_ISOLATION: Server validates organization IDs and scopes product reads and WebSocket event delivery; auth/membership enforcement is the next deployment layer
